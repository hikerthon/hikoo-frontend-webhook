...
const lineNotify = require('line-notify-nodejs')(
  'KSOzHZOdoR5RrcDIBFObgfZl48364bUuQvSolAdJPOq'
);
...
app.post('/pushed', (req, res) => {
  ...
  const deploy = exec(
    `./deploy ${repo} ${hash} hikoo-frontend`,
    (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        lineNotify
          .notify({
            message: `Deploy ${repo} ${hash} failed with message ${err.message}`,
          })
          .then(console.log)
          .catch(console.log);
        return;
  ...
  deploy.on('exit', (code) => {
    if (code !== 0) {
      lineNotify
        .notify({ message: `Deploy ${repo} ${hash} failed with code ${code}` })
        .then(console.log)
        .catch(console.log);
    ...
    lineNotify
      .notify({ message: `Deploy ${repo} ${hash} success` })
      .then(console.log)
      .catch(console.log);
